...
const AddTracksView: React.FC<AddTracksViewProps> = (
  ...
  const auth = useAuth();
  const match = useMatch("/music/playlists/:id/add-tracks/");
  ...
  React.useEffect(() => {
    const apiClient = new ApiClient(auth.session?.access);
    if (playlist && filters !== undefined && filtersKey === 'addMusicTracks') {
      const searchParams = new URLSearchParams(filters);
      apiClient.get(
        `/media/audio-tracks/`,
        {
          ...Object.fromEntries(searchParams.entries()),
          music_playlist: playlist.id
        }
      ).then(
        response => {
