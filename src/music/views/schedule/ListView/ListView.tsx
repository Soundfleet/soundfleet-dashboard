...
const ListView: React.FC<ListViewProps> = (
  ...
  React.useEffect(() => {
    ...
    if (filters !== undefined && filtersKey === 'musicSchedules') {
  ...
  const createDialog = () => (
    ...
    >
      <>
        <DialogTitle>Add new music schedule</DialogTitle>
        <DialogContent>
            <ScheduleForm 
              enabled
  ...
  return (
    <>
      ...
      <ConfirmationDialog 
        open={deleting !== null} 
        title="Delete music schedule"
        variant="error"
        onConfirm={() => handleDelete(deleting!)}
        onCancel={() => setDeleting(null)}
      >
        Are you sure you want to delete music schedule? This action is irreversible.
      </ConfirmationDialog>
      <Filters
        filtersKey="musicSchedules"
        filtersConf={[
          {
            queryParam: "search",
            filterParam: "search",
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} sx={{display: "flex"}}>
          <Box>
            <Typography variant="h4">Music schedules</Typography>
          </Box>
          <Box flexGrow={1}/>
          <Box>
            <Button 
                ...
                schedules.map((schedule: Schedule) => {
                  return (
                    <TableRow key={schedule.id}>
                      <TableCell>
                        <Link to={`/music/schedules/${schedule.id}/`}>{schedule.name}</Link>
                      </TableCell>
                      <TableCell
                        sx={{textAlign: "right"
                      }}>
