...
const ListView: React.FC<ListViewProps> = (
  ...
  React.useEffect(() => {
    ...
    if (filters !== undefined && filtersKey === 'adPlaylists') {
    ...
    <>
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} sx={{display: "flex"}}>
          <Box>
            <Typography variant="h4">Ad playlists</Typography>
          </Box>
          <Box flexGrow={1}/>
          <Box>
          <Button 
                ...
                playlists.map((playlist: Playlist) => {
                  return (
                    <TableRow key={playlist.id}>
                      <TableCell>
                        <Link to={`/ads/playlists/${playlist.id}/`}>{playlist.name}</Link>
                      </TableCell>
                      <TableCell>
                        <Chip
                          sx={{
